...
useEffect(() => {
  ...
  const longitude = Number(pickUpLon);

  if (Number.isNaN(latitude) || Number.isNaN(longitude)) return;

  getNearByVehicles(latitude, longitude, vehicle);
  ...
  return (
    <div className="min-h-screen bg-zinc-100 text-zinc-900 overflow-hidden">
      ...
      {/* MAP */}
      <div className="relative z-0 w-full h-[52vh]">
        <SearchMap
          pickUp={pickUp}
          drop={drop}
          onChange={(p, d) => {
      ...
      <motion.div
        ...
        className="relative z-20 bg-white rounded-t-[28px] border-t border-zinc-200 shadow-[0_-8px_40px_rgba(0,0,0,0.15)] pt-5 pb-20 min-h-[52vh]"
      >
        <div className="px-5 lg:px-8 max-w-6xl mx-auto">
          ...
          >
            <div className="flex gap-3 px-4 py-3 border-b border-zinc-100">
              <div className="flex flex-col items-center pt-1.5 flex-shrink-0">
                ...
                <div
                  className="w-px flex-1 bg-zinc-30 my-1"
                  style={{ minHeight: 14 }}
                />
              </div>
          ...
          >
            <div>
              <h2 className="text-zinc-900 text-lg font-black tracking-tight">
                {loading?"Finding vehicles":vehicles.length>0?"Avilable":"No nearby vehicles"}
              </h2>
              {
          ...
          <AnimatePresence>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grdi-cols-3 xl:grid-cols-4 gap-4">
              {vehicles.map((v,i) =>(
                <motion.div
                key={i}
